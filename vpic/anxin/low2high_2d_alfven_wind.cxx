//////////////////////////////////////////////////////
//
//   Coupling from low- to high-frequency waves
//
//////////////////////////////////////////////////////
...
begin_initialization {
  ...
  double betai_par = 0.1;   // Ratio of ion thermal to magnetic pressure.
  double Tperp_Tpar = 1.0;  // Initial temperature anisotropy for ions.
  double gamma = 1.0;   // Ratio of specific heats.
  double Lx_di = 120.0;      // Domain size in ion skin depth.
  double Ly_di = 15.0;      // Domain size in ion skin depth.
  double Tipar_Te = 0.58;    // Initial ratio of electron temperature to parallel ion temperature.
  // double cs = sqrt(0.5*betai_par*(0.4+1.0/Tipar_Te));              // Ion acoustic speed
  double pert = 0.15;
  double kx[3];
  double phi[3];
  double coeffufl[3];
  for (int i=0; i<3; i++) {
    kx[i] = (i+3) * 2.0 * M_PI / Lx_di;
    phi[i] = i * 2.0 * M_PI / 3.0;
  }
  coeffufl[0] = 1.082;
  coeffufl[1] = 1.110;
  coeffufl[2] = 1.139;
  ...
  double taui    = 800.0;    // Simulation run time in wci^-1.
  double quota   = 11.5;   // run quota in hours
  double quota_sec = quota*3600;  // Run quota in seconds
  
  double Lx    = Lx_di*di; // size of box in x dimension
  double Ly    = Ly_di*di;   // size of box in y dimension
  ...
  double nppc  = 900;         // Average number of macro particle per cell per species 
  ...
  double dt = 0.004/wci;                           // time step

  double sort_interval = 40;  // How often to sort particles
  
  // Intervals for output
  int restart_interval = int(10.0/dt);
  ...
  int Hparticle_interval = 10*interval;
  ...
  grid->nsub = 10;  // Number of substeps for field solve.
  grid->nsm= 0;    // Number of binomial smoothing passes (to fields & moments).
  grid->nsmb = 2000;  // Timesteps between additional smooths of magnetic field (0 is off).
  ...
  // sim_log ( "vp[0]/V_A = " << vp[0]/v_A);
  // sim_log ( "vp[1]/V_A = " << vp[1]/v_A);
  // sim_log ( "vp[2]/V_A = " << vp[2]/v_A);
...
// #define DBY (pert*kx[0]/ky[0]*(sin(kx[0]*x+ky[0]*y)))
// #define DBX (-pert*(sin(kx[0]*x+ky[0]*y)))
...
#define DBZ (pert*(cos(kx[0]*x+phi[0])+cos(kx[1]*x+phi[1])+cos(kx[2]*x+phi[2])))
  set_region_field( everywhere, 0, 0, 0, 1.0, DBY, DBZ );
  ...
  double zmin = grid->z0 , zmax = grid->z0+(grid->dz)*(grid->nz);
   ...
   repeat ( Ni/nproc() ) {
     ...
     for (int i=0; i<3; i++){
       uy += pert * sin(kx[i]*x+phi[i]) * coeffufl[i];
       uz -= pert * cos(kx[i]*x+phi[i]) * coeffufl[i];
     }
...
begin_diagnostics {
  ...
  if(step()==0) {
    ...
    dump_mkdir("rundata");
    dump_mkdir("restore1");
    dump_mkdir("restore2");  // 1st backup
    dump_mkdir("particle");
    dump_mkdir("rundata");
  ...
  *------------------------------------------------------------------------*/
  ...
  char subdir[36];
  if ( should_dump(Hparticle) && step() != 0 ){
    sprintf(subdir, "particle/T.%ld", (long)step());
    dump_mkdir(subdir);
    sprintf(subdir, "particle/T.%ld/Hparticle", (long)step());
    dump_particles("ion", subdir);
  }
