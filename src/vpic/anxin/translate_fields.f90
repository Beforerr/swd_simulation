...
program translate
  ...
  real(kind=4), allocatable, dimension(:,:,:) :: bx0,by0,bz0,rhob,exc,ezc,buffer,absJ,absB
  real(kind=4), allocatable, dimension(:,:,:,:) :: eb
  real(kind=4), allocatable, dimension(:,:,:) :: jfx, jfy, jfz, rhof, jfxold, jfyold, jfzold, rhofold
...
! read the configuration file
  ...
  if (myid==master) then
     open(unit=10,file='conf_fields.dat',form='formatted',status='old')
     read(10,datum)
     close(10)
  endif
...
! Allocate storage space for fields and moments
  ...
  allocate(jfx(ht%nx,ht%ny,ht%nz))
  allocate(jfy(ht%nx,ht%ny,ht%nz))
  allocate(jfz(ht%nx,ht%ny,ht%nz))

  allocate(rhof(ht%nx,ht%ny,ht%nz))
...
! Read in field data and load into global arrays
              ...
              read(10)buffer
              jfx(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              jfy(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              jfz(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              rhof(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)
